	...
	bfd_reloc_code_real_type code)
{
	...
	for (i=ARR_LEN(volt32_reloc_map); i--;)
	{
		if (volt32_reloc_map [i].bfd_reloc_val == code)
		{
			return &volt32_elf_howto_table
				[volt32_reloc_map[i].volt32_reloc_val];
		}
	}
...
volt32_reloc_name_lookup(bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
	...
	for (i=0; i<ARR_LEN(volt32_elf_howto_table); ++i)
	{
...
static bfd_boolean
volt32_info_to_howto_rela(bfd *abfd, arelent *cache_pt,
	Elf_Internal_Rela *dst)
{
	unsigned int r_type;

	r_type = ELF32_R_TYPE(dst->r_info);
	if (r_type >= (unsigned int)R_VOLT32_max)
	{
		/* xgettext:c-format */
