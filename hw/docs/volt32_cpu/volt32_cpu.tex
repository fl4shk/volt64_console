...
\section{Instructions}
		...
		\item Instruction List:
			...
			\item \texttt{shl dA, dB.addr, dC}
				...
				\item For \texttt{shl.v dA, dB.addr, dC}, this instruction
					...
					long) vector of element type \texttt{dA.type}.
				\item This instruction casts a temporary copy of
					\texttt{dC} to the unsigned type that is the same size
					as \texttt{dA}'s type and uses that instead of
					\texttt{dC}.
	...
	\subsection{Group 1 Instructions}
		...
		%	\texttt{simm12} (sign-extended to 32-bit), to calculate the
		%	address being loaded from. Also, for these same instructions,
		%	the type \texttt{dA} is set to is indicated in the instruction
		%	name, with, for example, \texttt{ldu8} setting \texttt{dA}'s
		%	type to \texttt{u8}.
		\item Instruction List:
			\begin{enumerate}
			%--------
	...
	\subsection{Group 5 Instructions}
		...
		\item For Encoding 0, the address to \texttt{fetch} from is
			computed by adding the address field of \texttt{iB} to the
			scalar data field (temporarily casted to the \texttt{u32} type)
		...
		%	\texttt{isimm11 << 2}.
		\item For Encoding 1, the address to \texttt{fetch} from is
			computed by adding the address field of \texttt{iB} to the
			the value \texttt{cast(s32, (isimm11 << 2))}.
