...
\section{Registers, Main Widths, etc.}
	...
	\item DLARs
		...
		\item In supervisor mode, DLARs 64 to 127 are referred to as
			...
			\texttt{d60}, \texttt{dcp}, \texttt{dfp}, \texttt{dsp}. Note
			that supervisor mode DLARs are encoded into instructions
...
\section{Instructions}
		...
		\item Instruction List:
			...
			\item \texttt{shl dA, dB.addr, dC}
				...
				\item For \texttt{shl.v dA, dB.addr, dC}, this instruction
					...
					into a temporary (DLAR data's length number of bytes
					long) vector of element type \texttt{dA.type} for the
			...
			\item \texttt{add dA, iB.addr, dC}
				...
				\item For \texttt{add.v dA, iB.addr, dC}, this instruction
					duplicates the value of \texttt{cast(dA.type, iB.addr)}
					(a scalar) into a temporary (DLAR data's length number
					of bytes long) vector of element type \texttt{dA.type}
					for the purposes of this calculation.
				%--------
	...
	\subsection{Group 1 Instructions}
		\begin{itemize}
		%--------
		\item Encoding: \texttt{0001 aaaa aabb bbbb  cccc cc00 000o oooo}
			\begin{itemize}
			%--------
			\item \texttt{a}: DLAR a
			\item \texttt{b}: DLAR b
			...
			%--------
			\end{itemize}
		\item For the first 8 instructions with names of the form
			"\texttt{ld<type>}", the \texttt{dB} DLAR's scalar data field
			(temporarily casted to the \texttt{u32} type) is added to
			the \texttt{dC} DLAR's scalar data field (temporarily casted to
			the \texttt{u32} type), to calculate the address being loaded
			from. Also, for these same instructions, the type \texttt{dA}
			is set to is indicated in the instruction name, with, for
			example, \texttt{ldu8} setting \texttt{dA}'s type to
			\texttt{u8}.
		\item For the second 8 instructions with names of the form
			"\texttt{ld<type>}", the \texttt{dB} DLAR's address field
			(temporarily casted to the \texttt{u32} type) is added to the
			\texttt{dC} DLAR's scalar data field (temporarily casted to the
			\texttt{u32} type), to calculate the address being loaded from.
			Also, for these same instructions, the type \texttt{dA} is set
			to is indicated in the instruction name, with, for example,
			\texttt{lds32} setting \texttt{dA}'s type to \texttt{s32}.
		\item For the instructions with names of the form
			"\texttt{st<type>}", the \texttt{dB} DLAR's scalar data field
			(temporarily casted to the \texttt{u32} type) is added to
			the \texttt{dC} DLAR's scalar data field (temporarily casted to
			the \texttt{u32} type), to calculate the address being stored
			to. Also, for these same instructions, the type \texttt{dA}
			is set to is indicated in the instruction name, with, for
			example, \texttt{sts64} setting \texttt{dA}'s type to
			\texttt{s64}.
		\item Instruction List:
			\begin{enumerate}
			%--------
	...
	\subsection{Group 2 Instructions}
		...
		\item Encoding: \texttt{0010 aaaa aabb bbbb  iiii iiii iiii iooo}
			\begin{itemize}
			%--------
			\item \texttt{a}: DLAR a
			\item \texttt{b}: DLAR b
		...
		\item For these instructions, the \texttt{dB} DLAR's scalar data
			field (temporarily casted to the \texttt{u32} type) is added to
			\texttt{simm13} (sign-extended to 32-bit), to calculate the
			address being loaded from.
		...
		\item Instruction List:
			...
			\item \texttt{ldu8 dA, dB, simm13}
			\item \texttt{lds8 dA, dB, simm13}
			\item \texttt{ldu16 dA, dB, simm13}
			\item \texttt{lds16 dA, dB, simm13}
			\item \texttt{ldu32 dA, dB, simm13}
			\item \texttt{lds32 dA, dB, simm13}
			\item \texttt{ldu64 dA, dB, simm13}
			\item \texttt{lds64 dA, dB, simm13}
			%--------
			\end{enumerate}
		%--------
		\end{itemize}
	...
	\subsection{Group 3 Instructions}
		...
		\item Encoding: \texttt{0011 aaaa aabb bbbb  iiii iiii iiii iooo}
			\begin{itemize}
			%--------
			\item \texttt{a}: DLAR a
			\item \texttt{b}: DLAR b
		...
		\item Instruction List:
			...
			\item \texttt{stu8 dA, dB, simm13}
			\item \texttt{sts8 dA, dB, simm13}
			\item \texttt{stu16 dA, dB, simm13}
			\item \texttt{sts16 dA, dB, simm13}
			\item \texttt{stu32 dA, dB, simm13}
			\item \texttt{sts32 dA, dB, simm13}
			\item \texttt{stu64 dA, dB, simm13}
			\item \texttt{sts64 dA, dB, simm13}
			%--------
			\end{enumerate}
		%--------
		\end{itemize}
	...
	\subsection{Group 5 Instructions}
		\begin{itemize}
		%--------
		\item Encoding 0: \texttt{0101 aaaa aabb bbbb  cccc cc00 000j jjjo}
		\item Encoding 1: \texttt{0101 aaaa aabb bbbb  iiii iiii iiij jjjo}
			\begin{itemize}
			%--------
			\item \texttt{a}: ILAR a
			\item \texttt{b}: ILAR b
	...
	\subsection{Group 6 Instructions}
		...
		\item Encoding: \texttt{0110 aaaa aabb bbbb  iiii ijjj jjvo oooo}
			\begin{itemize}
			%--------
			\item \texttt{a}: DLAR a
			\item \texttt{b}: ILAR b
	...
	\subsection{Group 7 Instructions}
		...
		\item Encoding: \texttt{0111 aaaa aabb bbbb  cccc ccii iiii Sooo}
			...
			\item \texttt{i}: \texttt{imm6} (zero-extended 6-bit
				immediate), amount of LARs to use
			\item \texttt{S}:
				\begin{itemize}
				%--------
		...
		\item Instruction List:
			...
			\item \texttt{swi dA, dB, dC, dD}
				...
				\item Note that this instruction always causes an exception
					to occur.
				\item This instruction stores \texttt{dA}'s vector data to
					into \texttt{swiarg0}.
				\item This instruction stores \texttt{dB}'s vector data to
					into \texttt{swiarg1}.
				\item This instruction stores \texttt{dC}'s vector data to
					into \texttt{swiarg2}.
				\item This instruction stores \texttt{dD}'s vector data to
					into \texttt{swiarg3}.
				%--------
				\end{itemize}
			\item \textit{Reserved for future expansion.}
	...
	%	\begin{itemize}
	%	%--------
	%	\item Encoding: \texttt{1011 aaaa aabb bbbb  iiii iiii iiii iooo}
	%		\begin{itemize}
	%		%--------
	%		\item \texttt{a}: DLAR a
	%		\item \texttt{b}: DLAR b
	...
	%		\begin{enumerate}
	%		%--------
	%		\item \texttt{ldu8 dA, dB, ipc.addr, simm13}
	%		\item \texttt{lds8 dA, dB, ipc.addr, simm13}
	%		\item \texttt{ldu16 dA, dB, ipc.addr, simm13}
	%		\item \texttt{lds16 dA, dB, ipc.addr, simm13}
	%		\item \texttt{ldu32 dA, dB, ipc.addr, simm13}
	%		\item \texttt{lds32 dA, dB, ipc.addr, simm13}
	%		\item \texttt{ldu64 dA, dB, ipc.addr, simm13}
	%		\item \texttt{lds64 dA, dB, ipc.addr, simm13}
	%		%--------
	%		\end{enumerate}
	%	%--------
	%	\end{itemize}
