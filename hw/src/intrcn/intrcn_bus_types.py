...
class IntrcnBstSize(Enum):
	...
	def __int__(self):
		return 1 << self.value
class IntrcnBstType(Enum):
	FIXED = 0b0
	INCR = 0b1
#--------
class IntrcnResp(Enum):
	OKAY = 0b00
	RESERVED = 0b01
	SLVERR = 0b10
	DECERR = 0b11
...
class IntrcnNodeBus:
	...
	def __init__(self, ADDR_WIDTH, DATA_WIDTH, *, name_prefix=""):
		...
		self.who.bwidth = Signal(shape=Shape.cast(IntrcnBstWidth),
		...
		self.who.btype = Signal(shape=Shape.cast(IntrcnBstType),
			name=psconcat(name_prefix, "who_btype"))


		# Write response (host input, device output)
		self.whi.resp = Signal(name=psconcat(name_prefix, "whi_resp"))
		...
		self.rho.bwidth = Signal(shape=Shape.cast(IntrcnBstWidth),
		...
		self.rho.btype = Signal(shape=Shape.cast(IntrcnBstType),
		...
		self.rhi.resp = Signal(name=psconcat(name_prefix, "rhi_resp"))
		#--------
	#--------
	def ADDR_WIDTH(self):
